...
void swapCardFromDeck  (int x, int temp, cardInfo generatedDeck[]);
void shuffleMyDeck (int deckSize ,cardInfo generatedDeck[]);
...
void winner (gameData loadGame, char color[][MAX_LENGTH_OF_WORD_COLOR], int **colorCounter, bool **immunity);
...
void infoAboutCard (gameData myDeck, players playersStats[], cauldrons cauldronsData[], int values[], int index[], int &cardValue, char cardColor[], cardInfo smallestCopy[], cardInfo smallest[]);
...
void mostDangerousCard (gameData myDeck, players playersStats[], cauldrons cauldronsData[], cardInfo biggest[], cardInfo biggestCopy[]);
void betterInfoAboutCard (gameData &myDeck, players playersStats[], cauldrons cauldronsData[], int values[], int index[], int &cardValue, char cardColor[], cardInfo smallestCopy[], cardInfo smallest[], cardInfo piles[]);
void betterSmallestCardMove (gameData &myDeck, players playersStats[], cauldrons cauldronsData[]);
void tests();
...
//wczytywanie parametrow i generowanie talii
    gameData myDeck{};
    myDeck.activePlayer = 1;
    insertDeckData(myDeck);
    int deckSize = myDeck.greenCards + (myDeck.cauldrons * myDeck.otherCardValue);
    int whichPlayer = deckSize % myDeck.players; //zmienna okreslajacy ktory gracz jako ostatni rzuca karty w grze
    if (whichPlayer == 0)
    ...
    shuffleMyDeck(deckSize, generatedDeck); //tasowanie kart
    ...
    std::cin >> myNumber;
    //gracz numer 1 rozdaje karty generuje plik i pokazuje 0 stan gry
    if (myNumber == 1) {
    ...
    while (true)
    {
        //dopoki ostatni gracz ma karty na rece
        if (playersStats[whichPlayer -1].playerCardsOnHand[0].value != 0) {
            loadGameFromFile(myDeck, playersStats, cauldronsData);
            if (myDeck.activePlayer == myNumber) {
                std::cout << "Runda: " << iterator << std::endl;
                if (isGameOk(myDeck, cauldronsData, playersStats) && myNumber == 2) {
                    easyMove(myDeck, cauldronsData, playersStats); // gracz numer dwa gra algorytmem rzucajacym losowe pierwsze z brzegu karty
                    std::cout << "My turn is done..." << std::endl;
                } else if (isGameOk(myDeck, cauldronsData, playersStats) && myNumber == 1) {
                    smallestCardMove(myDeck, playersStats, cauldronsData); // gracz numer jeden gra algorytmem rzucajacym najmniejsza karte
                    std::cout << "My turn is done..." << std::endl;
                } else if (isGameOk(myDeck, cauldronsData, playersStats) && myNumber == 3) {
                    // gracz numer trzy gra algorytmem rzucajacym najmniejsze karty chyba ze ma nastapic wybuch kociolka to pozbywa sie najgrozniejszej karty
                    betterSmallestCardMove(myDeck, playersStats, cauldronsData);
                    std::cout << "My turn is done..." << std::endl;
                } else
                    break;
...
void shuffleMyDeck (int deckSize ,cardInfo generatedDeck[])
{
    ...
    for (int i = 0; i < deckSize; i++)
    {
        ...
        swapCardFromDeck (i, temp, generatedDeck);
    }
}
...
void cardsDeal (players info[], cardInfo deck[], gameData myDeck, int deckSize)
{
    ...
    for (int i = 0; i < deckSize; i++)
    {
        //indeks gracza dostaje z operacji modulo iterator przez ilosc graczy
        ...
        //zwiekszam ilosc kart na rece gracza
...
void activeAndHowMany(gameData &loadedGame)
{
    ...
    //najpierw wczytuje pierwsze trzy linijki z pliku
    int index = 0;
    if (fptr != NULL) {
        while(index < 3)
        {
    ...
    {
        //indeks zerowy zmiennej firstInput podaje kto jest aktualnym aktywnym graczem
        ...
        //indeks numer jeden zmiennej firstInput podaje ilu jest graczy
        ...
        //indeks numer dwa zmiennej firstInput podaje przekroczenie jakiej liczby wywoluje wybuch kociolka
    ...
    //przypisuje dane do struktury danych gry
...
void gettingRestInput (gameData loadedGame, players playersData[])
{
    ...
    if (fptr != NULL) {
        ...
        {
            if (index <= 2) //odrzucam pierwsze 3 linijki z pliku, ponieważ zajalem sie nimi w poprzedniej funkcji
                fgets(trash[index], MAX_LENGTH_LINE_INPUT, fptr);
            if (index > 2) //wczytuje dane na temat graczy
            {
    ...
    for (int i = 0; i < 2 * loadedGame.players; i++)
    {
        ...
        {
            char temp[2];
            if (i % 2 == 0) //jezeli prawda zajmuje sie kartami gracza na rece
            {
                if (inputLines[i][j] >= '0' && inputLines[i][j] <= '9' && inputLines[i][j + 1] >= '0' && inputLines[i][j + 1] <= '9' && j < MAX_LENGTH_LINE_INPUT-1) //dwucyfrowe wartosci kart
                {
                ...
                else if (inputLines[i][j] >= '0' && inputLines[i][j] <= '9' && inputLines[i][j-1] == ' ') //jednocyfrowe wartosci kart
                {
            ...
            else if (i % 2 != 0) //jezeli prawda zajmuje sie kartami gracza na stoliku
            {
                if (inputLines[i][j] >= '0' && inputLines[i][j] <= '9' && inputLines[i][j + 1] >= '0' && inputLines[i][j + 1] <= '9' && j < MAX_LENGTH_LINE_INPUT-1) //dwucyfrowe wartosci kart
                {
                    temp[0] = inputLines[i][j];
...
void howMuchCauldrons(gameData &loadedGame, char **input)
{
    ...
    if (fptr != NULL) {
        while(!feof(fptr))
        {
            if (index < maxArraySize) //odrzucam poczatek pliku tj. dane dot. ilosc graczy i dane na temat kart graczy
                fgets(trash[index], MAX_LENGTH_LINE_INPUT, fptr);
            if (index > maxArraySize-1)  //przypisuje do zmiennej linijki dotyczace kart na kociolkach
            {
    ...
    loadedGame.cauldrons = cauldronsCounter; //na podstawie wczytanych linijek obliczam ile jest kosciolkow w grze
}
...
void cauldronsDataFromInput (char **input, cauldrons cauldronsData[], gameData loadedGame)
{
    for (int i = 0; i < loadedGame.cauldrons; i++)
    {
        ...
        {
            char temp[2];
            if (input[i][j] >= '0' && input[i][j] <= '9' && input[i][j + 1] >= '0' && input[i][j + 1] <= '9' && j < MAX_LENGTH_LINE_INPUT-1) //jezeli wartosc karty jest dwucyfrowa
            {
                temp[0] = input[i][j];
                temp[1] = input[i][j + 1];
            ...
            else if (input[i][j] >= '0' && input[i][j] <= '9' && input[i][j-1] == ' ') //jezeli wartosc karty jest jednocyfrowa
            {
...
//Wyswietlanie komunikatu bledu jezeli w grze znajduja sie rozne ilosci kart danych kolorow
void printFridayThe13th4bCheckGreenCards(int cards[])
{
    char cardColors[6][MAX_LENGTH_OF_WORD_COLOR] = {"blue", "red", "violet", "yellow", "white", "black"};
    bool equal = true;
    int temp = 0;
    for (int i = 0; i < 6; i++) //iteruje po ilosci kolorow kart
    {
        if (cards[i] != 0)
        {
            temp = cards[i];
...
bool fridayThe13th4bCheckGreenCards (gameData loadedGame, cauldrons cauldronsData[], players playersData[])
{
    ...
    for (int i = 0; i < 6; i++)
        cards[i] = 0;
    fridayThe13th4bCheckGreenCardsPl1(loadedGame, playersData, cards); //sprawdza karty na rece gracza
    fridayThe13th4bCheckGreenCardsPl2(loadedGame, playersData, cards); //sprawdza karty na stoliku gracza
    fridayThe13th4bCheckGreenCardsCo(loadedGame, cauldronsData, cards); //sprawdza karty w kocilkach
...
void differentColors (gameData loadGame, cauldrons cauldronsData[], int color[])
{
    char *temp = new char[loadGame.cauldrons]; //tablica przechowywujaca kolor kociolka
    for (int i = 0; i < loadGame.cauldrons; i++)
        temp[i] = '0';
    for (int i = 0; i < loadGame.cauldrons; i++)
    {
...
void explosiveTreshold (gameData loadGame, cauldrons cauldronsData[], int explosive[])
{
    ...
    for (int i = 0; i < loadGame.cauldrons; i++)
    {
        for (int j = 0; j < MAX_CARDS_ON_PLAYER_DECK_OR_HAND; j++)
        {
            boom[i] += cauldronsData[i].cardsOnPile[j].value; //sprawdzam sume wartosci kart na kociolku
        }
    }
    for (int i = 0; i < loadGame.cauldrons; i++)
    {
...
bool checkAmountOfCards (gameData loadGame, players playersData[])
{
    ...
    {
        if (i - 1 == loadGame.activePlayer - 1) //jezeli jest active playerem to roznica kart moze byc maksymalnie dwa
            {
            if (abs(playersData[i - 1].handSize - playersData[i].handSize) > 2)
                return false;
            }
        else //jezeli nie jest active playerem to roznica kart moze byc maksymalnie 1
...
void addToPile (gameData loadGame, cauldrons cauldronsData[], int cardValue, char cardColor[])
{
    bool isEmpty = true; //jezeli w kociolku lezy inna karta niz zielona to wartosc bedzie wynosila false
    for (int i = 0; i < loadGame.cauldrons; i++) {
        for (int j = 0; j < MAX_CARDS_ON_PLAYER_DECK_OR_HAND; j++) {
    ...
    if (isEmpty || cardColor[0] == 'g') //jezeli karta jest zielona dodaje do pierwszego kociolka
    {
        for (int i = 0; i < MAX_CARDS_ON_PLAYER_DECK_OR_HAND; i++) {
    ...
    } else //jezeli kolor kart na stosie zgadza sie z kolorem karty do dodania
        {
        int index = -1;
        for (int i = 0; i < loadGame.cauldrons; i++) {
            for (int j = 0; j < MAX_CARDS_ON_PLAYER_DECK_OR_HAND; j++) {
        ...
        if (index != -1) //dodaje karte do uprzednio wybranego kociolka
        {
            for (int i = 0; i < MAX_CARDS_ON_PLAYER_DECK_OR_HAND; i++) {
        ...
        } else //jezeli w zadnym kociolku nie wystepuje kolor karty do dodania szuka indeksu pustego kociolka
            {
            int temp = -1;
            for (int i = 0; i < loadGame.cauldrons; i++) {
                bool empty = true;
...
void checkImmunity (gameData loadGame, players playersData[], int **colorCounter, char color[][MAX_LENGTH_OF_WORD_COLOR], bool **immunity)
{
    for (int i = 1; i < 7; i++) //iteruje po kolorach i licze ktory gracz ma najwiecej kart danego koloru
    {
        int max = 0;
        int maxId = -1;
        int j = 0;
        while (j < loadGame.players)
        {
            ...
            else if (temp == max) //jezeli istnieje kilka wartosci maksymalnych nikt nie ma immunitetu
            {
                maxId = -1;
            }
            j++;
...
//Dodaje do pliku wynik rozgrywki
void winner (gameData loadGame, char color[][MAX_LENGTH_OF_WORD_COLOR], int **colorCounter, bool **immunity)
{
    int *score = new int[loadGame.players];
    for (int i = 0; i < loadGame.players; i++)
...
//    winner(loadGame, color, colorCounter, immunity);
...
void loadGameFromFile (gameData &myDeck, players playersStats[], cauldrons cauldronsData[])
{
    ...
    for (int i = 0; i < myDeck.players; i++) //czyszczenie tablicy graczy
    {
        for (int j = 0; j < MAX_CARDS_ON_PLAYER_DECK_OR_HAND; j++)
        {
            playersStats[i].playerCardsOnDeck[j].value = 0;
    ...
    for (int i = 0; i < myDeck.cauldrons; i++) //czyszczenie tablicy kociolkow
        for (int j = 0; j < MAX_CARDS_ON_PLAYER_DECK_OR_HAND; j++)
        {
            cauldronsData[i].cardsOnPile[j].value = 0;
            for (int p = 0; p < MAX_LENGTH_OF_WORD_COLOR; p++)
...
/*
 * algorytm pozbywajacy sie najmniejszych kart do kociolkow o najmniejszych wartosciach
 */

//wybiera tablice kart o najmniejszych wartosciach
void whichCards (gameData myDeck, players playersStats[], cauldrons cauldronsData[], cardInfo smallest[], cardInfo smallestCopy[])
{
    int index = 1;
    for (int i = 0; i < MAX_CARDS_ON_PLAYER_DECK_OR_HAND; i++) //segreguje karty na rece od najmniejszej do najwiekszej
    {
    ...
    for (int i = 1; i < MAX_CARDS_ON_PLAYER_DECK_OR_HAND; i++) //przypisuje do tablicy karty o najmnieszej wartosci
    {
...
//dodaje do kazdego kociolka jego kolor i sume kart
void whichPile (gameData myDeck, players playersStats[], cauldrons cauldronsData[], cardInfo piles[])
{
    for (int i = 0; i < myDeck.cauldrons; i++)
    {
...
//znajduje najmniejsza sume kociolka po dodaniu do niego karty
void addSmallestCardToPile (gameData myDeck, players playersStats[], cauldrons cauldronsData[], cardInfo piles[], cardInfo smallest[], int values[], int index[])
{
    int min = 10000, id = -1; //szuka najmniejszej wartosci kociolka i bierze jego id
    for (int i = 0; i < myDeck.cauldrons; i++) {
        if (min > piles[i].value) {
            min = piles[i].value;
            id = i;
        }
    }
    for (int i = 0; i < myDeck.cauldrons; i++) {
        for (int j = 0; j < MAX_CARDS_ON_PLAYER_DECK_OR_HAND; j++)
        { //jezeli karta do dodania ma kolor zielony dodaje znajduje najmniejszy kociolek i wykresla karte z listy
            if (i == id && smallest[j].color[0] == 'g') {
                values[i] = smallest[j].value + piles[i].value;
                index[i] = j;
                smallest[j].value = 0;
                for (int p = 0; p < MAX_LENGTH_OF_WORD_COLOR; p++)
                    smallest[j].color[p] = 0;
            } //jezeli kolor kociolka zgadza sie z kolorem karty bierze id kociolka i wykresla z listy
    ...
    } //jezeli nie wystepuje kociolek o kolorze karty znajduje pusty kociolek i wykresla karte z listy
    for (int i = 0; i < myDeck.cauldrons; i++) {
        for (int j = 0; j < MAX_CARDS_ON_PLAYER_DECK_OR_HAND; j++)
        {
            if (piles[i].color[0] == 0 && smallest[j].value != 0) {
                values[i] = smallest[j].value + piles[i].value;
                index[i] = j;
                smallest[j].value = 0;
                for (int p = 0; p < MAX_LENGTH_OF_WORD_COLOR; p++)
...
//wybiera karte do dodania i dodaje do kociolka
void infoAboutCard (gameData myDeck, players playersStats[], cauldrons cauldronsData[], int values[], int index[], int &cardValue, char cardColor[], cardInfo smallestCopy[], cardInfo smallest[])
{
    int id = 1;
    int minId = -1;
    int temp = 100000;
    for (int i = 0; i < myDeck.cauldrons; i++) // szuka najmniejszego kociolka po dodaniu do niego karty
    {
    ...
    for (int i = 0; i < MAX_CARDS_ON_PLAYER_DECK_OR_HAND; i++) //dodaje karte do kociolka
    {
        if (cauldronsData[id].cardsOnPile[i].value == 0){
            cauldronsData[id].cardsOnPile[i].value = cardValue;
            for (int j = 0; j < MAX_LENGTH_OF_WORD_COLOR; j++)
                cauldronsData[id].cardsOnPile[i].color[j] = cardColor[j];
            break;
        }
    }
}

//usuwa dodana karte z reki gracza
void deleteCardFromHand (gameData myDeck, players playersStats[], int cardValue, char cardColor[])
{
    for (int i = 0; i < MAX_CARDS_ON_PLAYER_DECK_OR_HAND; i++)
    {
...
//rzuca najmniejsza karte do kociolka o najmniejszej sumie kart
void smallestCardMove (gameData &myDeck, players playersStats[], cauldrons cauldronsData[])
{
    ...
    infoAboutCard (myDeck, playersStats, cauldronsData, values, index, cardValue, cardColor, smallestCopy, smallest);
...
/*
 * algorytm rzucajacy najmniejsze karty, chyba ze ma nastapic wybuch kociolka to pozbywa sie najwiekszej karty
 */

//sortuje karty od najwiekszej do najmniejszej i wybiera tablice najwiekszych kart
void mostDangerousCard (gameData myDeck, players playersStats[], cauldrons cauldronsData[], cardInfo biggest[], cardInfo biggestCopy[])
{
    int index = 1;
    for (int i = 0; i < MAX_CARDS_ON_PLAYER_DECK_OR_HAND; i++)
    ...
    biggest[0] = playersStats[myDeck.activePlayer - 1].playerCardsOnHand[0];
    biggestCopy[0] = playersStats[myDeck.activePlayer - 1].playerCardsOnHand[0];
    for (int i = 1; i < MAX_CARDS_ON_PLAYER_DECK_OR_HAND; i++)
    {
        if (playersStats[myDeck.activePlayer - 1].playerCardsOnHand[i].value == biggest[0].value) {
            biggest[index] = playersStats[myDeck.activePlayer - 1].playerCardsOnHand[i];
            biggestCopy[index] = playersStats[myDeck.activePlayer - 1].playerCardsOnHand[i];
            index++;
        }
    }
}

//jezeli rzut najmniejszej karty ma wybuchnac kociolek szuka najniebezpieczniejszej karty do rzucenia
void betterInfoAboutCard (gameData &myDeck, players playersStats[], cauldrons cauldronsData[], int values[], int index[], int &cardValue, char cardColor[], cardInfo smallestCopy[], cardInfo smallest[], cardInfo piles[])
{
    int id = 1;
    int minId = -1;
    ...
    for (int i = 0; i < MAX_LENGTH_OF_WORD_COLOR; i++)
        cardColor[i] = smallestCopy[minId].color[i];
    if (temp > myDeck.explosion) //jezeli rzut najmniejszej karty ma wybuchnac kociolek wywoluje funkcje jeszcze raz tym razem dla najwiekszej karty
    {
        for (int i = 0; i < myDeck.cauldrons; i++) {
            values[i] = 0;
            index[i] = 0;
        ...
        for (int i = 0; i < MAX_CARDS_ON_PLAYER_DECK_OR_HAND; i++) {
            smallest[i].value = 0;
            smallestCopy[i].value = 0;
            for (int j = 0; j < MAX_LENGTH_OF_WORD_COLOR; j++) {
                smallest[i].color[j] = 0;
                smallestCopy[i].color[j] = 0;
            }
        }
        mostDangerousCard (myDeck, playersStats, cauldronsData, smallest, smallestCopy);
        whichPile (myDeck, playersStats, cauldronsData, piles);
        addSmallestCardToPile (myDeck, playersStats, cauldronsData, piles, smallest, values, index);
        id = 1;
        minId = -1;
...
//algorytm rzucajacy najmniejsza karte, chyba ze ma nastapic wybuch kociolka to wybiera najniebezpieczniejsza karte i sie jej pozbywa
void betterSmallestCardMove (gameData &myDeck, players playersStats[], cauldrons cauldronsData[])
{
    int cardValue, pileId = -1;
    char cardColor[MAX_LENGTH_OF_WORD_COLOR];
    ...
    whichPile (myDeck, playersStats, cauldronsData, piles);
    addSmallestCardToPile (myDeck, playersStats, cauldronsData, piles, smallest, values, index);
    betterInfoAboutCard (myDeck, playersStats, cauldronsData, values, index, cardValue, cardColor, smallestCopy, smallest, piles);
    deleteCardFromHand (myDeck, playersStats, cardValue, cardColor);
    cauldronExplosion(myDeck, cauldronsData, playersStats, pileId);
    if (pileId != -1)
        addCardsAfterBoom(myDeck, cauldronsData, playersStats, pileId);
...
//wykonuje 1000 testow dla 3 graczy, kazdy z nich gra inna taktyka, wyniki zapisuje do pliku
void tests()
{
    for (int _; _ < 1000; _++) {
        gameData myDeck{};
        ...
        shuffleMyDeck(deckSize, generatedDeck);
        cauldrons *cauldronsData = new cauldrons[myDeck.cauldrons];
        for (int i = 0; i < myDeck.cauldrons; i++)
            for (int j = 0; j < MAX_CARDS_ON_PLAYER_DECK_OR_HAND; j++) {
                cauldronsData[i].cardsOnPile[j].value = 0;
        ...
        while (true) {
            if (playersStats[whichPlayer - 1].playerCardsOnHand[0].value != 0) {
                loadGameFromFile(myDeck, playersStats, cauldronsData);
                if (isGameOk(myDeck, cauldronsData, playersStats) && myDeck.activePlayer == 2) {
                    easyMove(myDeck, cauldronsData, playersStats);
//                    std::cout << std::endl;
                } else if (isGameOk(myDeck, cauldronsData, playersStats) && myDeck.activePlayer == 1) {
                    smallestCardMove(myDeck, playersStats, cauldronsData);
//                    std::cout << std::endl;
                } else if (isGameOk(myDeck, cauldronsData, playersStats) && myDeck.activePlayer == 3) {
                    betterSmallestCardMove(myDeck, playersStats, cauldronsData);
//                    std::cout << std::endl;
                }
                generateGameDataFile(myDeck, cauldronsData, playersStats);
